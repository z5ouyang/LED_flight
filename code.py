...
TEXT_COLOR = [
    ...
    0x0040B0,
    0xFFBF00,
]  # ;g = get_text(labels_s);matrixportal.display.root_group = g
...
def get_matrix_portal():
    ...
    matrixportal = MatrixPortal(rotation=0, debug=False, bit_depth=6)
    return matrixportal
...
def log_init():
    ...
    try:
        if os.getenv("MQTT_BROKER_IP") is not None and os.getenv("MQTT_BROKER_PORT") is not None:
            print(f"{os.getenv("MQTT_BROKER_IP")} \t {os.getenv("MQTT_BROKER_PORT")}")
            MQTT_LOG = MQTT.MQTT(
...
def log(msg):
    ...
    try:
        ...
        MQTT_LOG.publish("logs/matrixportal", msg, qos=0)
        # print(f"{now.tm_year}-{now.tm_mon:02}-{now.tm_mday:02} {now.tm_hour:02}:{now.tm_min:02}:{now.tm_sec:02}","MQTT log", msg)
    except Exception as e:
        print(
            f"{now.tm_year}-{now.tm_mon:02}-{now.tm_mday:02} {now.tm_hour:02}:{now.tm_min:02}:{now.tm_sec:02}",
            "MQTT Error",
        )
        traceback.print_exception(type(e), e, e.__traceback__)
...
def display_date_time(time_within, matrixportal):
    ...
    labels = [
        month_names[now.tm_mon] + " " + str(now.tm_mday),
    ...
    for i in range(len(label_s)):
        g.append(
            adafruit_display_text.label.Label(
                FONT, color=text_col[i], x=1, y=i * 10 + 5, text=label_s[i]
            )
        )
    return g
...
def update_flight(flight_short, matrixportal, flip_east_west=None):
    if flight_short is None or flight_short["heading"] is None or flight_short["heading"] == "":
        return
    labels_s = [
        flight_short["flight_number"],
        flight_short["ori"] + "-" + flight_short["dest"],
        str(flight_short["altitude"]) + " ft",
    ]
    g = get_text(labels_s)
    heading = (
        (360 - int(flight_short["heading"])) % 360
        if flip_east_west
        else int(flight_short["heading"])
    )
...
def main():
    ...
    while True:
        ...
        if DEBUG_VERBOSE:
            ...
            log(
                f"Main loop: {(time.monotonic()-start_loop)*1000}ms\tAllocated: {gc.mem_alloc()} bytes\tFree: {gc.mem_free()} bytes"
            )
