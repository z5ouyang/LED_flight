...
def nearest(node, target, depth=0, best=None, dimensions=2):
    if node is None:
        return best
    if best is None or distance_sq(target, node.point, dimensions) < distance_sq(target, best, dimensions):
        best = node.point
    axis = depth % dimensions
    next_branch = None
    ...
    if (target[axis] - node.point[axis])**2 < distance_sq(target, best, dimensions):
